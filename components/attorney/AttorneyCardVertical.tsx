import Link from 'next/link';
import { Star, MapPin, Award } from 'lucide-react';
import { AttorneyWithDetails } from '@/lib/types/database';

interface AttorneyCardVerticalProps {
  attorney: AttorneyWithDetails;
}

export function AttorneyCardVertical({ attorney }: AttorneyCardVerticalProps) {
  const getTierBadge = (tier: string) => {
    switch (tier) {
      case 'exclusive':
        return {
          text: 'Featured',
          className: 'bg-yellow-100 text-yellow-800 border-yellow-200',
        };
      case 'standard':
        return {
          text: 'Standard',
          className: 'bg-blue-100 text-blue-800 border-blue-200',
        };
      default:
        return {
          text: 'Free',
          className: 'bg-gray-100 text-gray-800 border-gray-200',
        };
    }
  };

  const tierBadge = getTierBadge(attorney.membership_tier);

  // Defensive handling for experience years
  const experienceYears = attorney.experience_years || (attorney as any).years_experience;

  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 hover:shadow-md transition-shadow duration-200 h-full flex flex-col">
      <div className="p-6 flex flex-col h-full">
        {/* Attorney Photo - Centered at top */}
        <div className="flex justify-center mb-4">
          {attorney.profile_image_url ? (
            <img
              src={attorney.profile_image_url}
              alt={`${attorney.first_name || ''} ${attorney.last_name || ''}`.trim() || 'Attorney profile'}
              className="w-20 h-20 rounded-full object-cover"
            />
          ) : (
            <div className="w-20 h-20 rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white font-bold text-xl">
              {(attorney.first_name?.[0] || '')}{(attorney.last_name?.[0] || '')}
            </div>
          )}
        </div>

        {/* Attorney Info - Centered */}
        <div className="text-center flex-1 flex flex-col">
          {/* Name and Badges */}
          <div className="mb-2">
            <h3 className="text-lg font-semibold text-gray-900 mb-2">
              {attorney.first_name} {attorney.last_name}
            </h3>
            
            <div className="flex justify-center space-x-2 mb-2">
              {/* Membership Tier Badge */}
              <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium border ${tierBadge.className}`}>
                {tierBadge.text}
              </span>
              
              {/* Verified Badge */}
              {attorney.is_verified && (
                <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800 border border-green-200">
                  <Award className="h-3 w-3 mr-1" />
                  Verified
                </span>
              )}
            </div>
          </div>

          {/* Firm Name */}
          {attorney.firm_name && (
            <p className="text-sm text-gray-600 mb-3">{attorney.firm_name}</p>
          )}

          {/* Location */}
          {attorney.city && attorney.state && (
            <div className="flex items-center justify-center text-gray-600 mb-3">
              <MapPin className="h-4 w-4 mr-1" />
              <span className="text-sm">
                {attorney.city}, {attorney.state}
              </span>
            </div>
          )}

          {/* Primary Practice Area */}
          {((attorney as any).practice_categories && (attorney as any).practice_categories.length > 0) ? (
            <div className="mb-3">
              <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-lg px-3 py-2">
                <span className="font-medium text-blue-900 text-sm">
                  {(attorney as any).practice_categories[0].name}
                </span>
              </div>
            </div>
          ) : attorney.practice_areas && attorney.practice_areas.length > 0 ? (
            <div className="mb-3">
              <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-lg px-3 py-2">
                <span className="font-medium text-blue-900 text-sm">
                  {attorney.practice_areas[0].name || 'Legal Services'}
                </span>
              </div>
            </div>
          ) : null}

          {/* Experience */}
          {experienceYears && (
            <p className="text-sm text-gray-600 mb-3">
              {experienceYears}+ years experience
            </p>
          )}

          {/* Google Rating */}
          <div className="mb-4">
            {(attorney as any).google_rating && (attorney as any).google_review_count ? (
              <div className="flex items-center justify-center">
                <div className="flex items-center text-yellow-500">
                  {[...Array(5)].map((_, i) => (
                    <Star
                      key={i}
                      className={`h-4 w-4 ${
                        i < Math.floor((attorney as any).google_rating || 0)
                          ? 'fill-current'
                          : 'text-gray-300'
                      }`}
                    />
                  ))}
                </div>
                <span className="ml-2 text-sm text-gray-600">
                  {(attorney as any).google_rating.toFixed(1)} ({(attorney as any).google_review_count})
                </span>
              </div>
            ) : (
              /* Fallback for test data - show sample rating */
              <div className="flex items-center justify-center">
                <div className="flex items-center text-yellow-500">
                  {[...Array(5)].map((_, i) => (
                    <Star
                      key={i}
                      className={`h-4 w-4 ${
                        i < 4 ? 'fill-current' : 'text-gray-300'
                      }`}
                    />
                  ))}
                </div>
                <span className="ml-2 text-sm text-gray-600">
                  4.2 (12 reviews)
                </span>
              </div>
            )}
          </div>

          {/* View Profile Button - At bottom */}
          <div className="mt-auto">
            <Link
              href={`/attorney/${attorney.id}`}
              className="w-full bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-blue-700 transition-colors duration-200 shadow-sm inline-block text-center"
            >
              View Profile
            </Link>
          </div>
        </div>
      </div>
    </div>
  );
}
